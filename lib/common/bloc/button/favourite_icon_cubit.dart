...
class FavoriteIconCubit extends Cubit<FavoriteIconState> {
  FavoriteIconCubit(String productId)
      : super(FavoriteIconState(isFavorite: false, productId: productId));
  void isFavorite(String productId) async {
    var result = await sl<IsFavoriteUseCase>().call(params: productId);
    if (!isClosed) emit(state.copyWith(isFavorite: result));
  }
 ...
 if (!isClosed) {
        emit(state.copyWith(isFavorite: data));
      }
      if (!isClosed) {
